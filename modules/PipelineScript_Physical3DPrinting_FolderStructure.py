...
class FolderStructureCreator:
    def __init__(self, root):
        ...
        personal_check = ttk.Checkbutton(project_type_frame, text="Personal", variable=self.personal_var, 
                                       command=self.toggle_project_type)
        personal_check.grid(row=0, column=1, sticky="w", padx=5, pady=(0, 10))
        
        # alles3d checkbox
        self.alles3d_var = tk.BooleanVar(value=False)
        alles3d_check = ttk.Checkbutton(project_type_frame, text="alles3d", variable=self.alles3d_var, 
                                      command=self.toggle_project_type)
        alles3d_check.grid(row=0, column=2, sticky="w", padx=20, pady=(0, 10))
        ...
        self.alles3d_var.trace_add("write", lambda *args: self.update_preview())
    ...
    def toggle_project_type(self):
        """Toggle project type checkboxes and update related fields"""
        # Ensure only one project type is selected at a time
        if self.personal_var.get() and self.alles3d_var.get():
            # If both are checked, keep the one that triggered this call
            caller = self.root.focus_get()
            if caller and "alles3d" in str(caller):
                self.personal_var.set(False)
            else:
                self.alles3d_var.set(False)
        
        # Save current values if switching to a special project type
        if (self.personal_var.get() or self.alles3d_var.get()) and not hasattr(self, 'client_name_backup'):
    ...
    def get_folder_structure_preview(self):
        ...
        # Software-specific folders in Production (capitalized to match template)
        ...
        if self.use_affinity_var.get():
            folders.append(f"{production_folder}/Affinity")
    ...
    def update_preview(self):
        ...
        if self.personal_var.get() or self.alles3d_var.get():
    ...
    def create_folder_structure(self, project_dir, date, software_specs):
        ...
        if 'Affinity' in software_specs:
            folders_to_create.append(f"{production_folder}/Affinity")
